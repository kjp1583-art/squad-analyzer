# 🧥 브장신 파츠 생성 프롬프트 — 새 원화 기준

> 👗 **2026-09-10 방향 확정**: 의상·헤어·얼굴이 바뀌는 것은 **전신 스킨**으로 통째로 뽑는다(정면·곁눈·옆모습 3장, 편집 모드).
> 레이어(차분 추출)로 얹는 것은 **모자·장신구·소품·효과**만. 아래 "의상(o)" 표의 항목은 `/스킨추가` 로 올린다 — 프롬프트는 같되
> 3시점 모두 뽑고, 봇이 차분을 내지 않고 그림 그대로 스킨으로 쓴다(배경 키잉·정합만).
