...
_DRAFT_RULES = (
    ...
    "   특정 스킬의 메커니즘이 확실치 않으면 그것을 근거로 단정하지 말고, [현 패치 티어리스트]와 판수 데이터를 우선하라.\n\n"
    "【명장의 사고 순서 — 반드시 이대로】\n"
    ...
    "7) [현 패치 티어리스트]가 주어지면 동급 후보 중 티어 높은 쪽 우선. 클랜 내전 메타는 표본이 작으니 참고만(1~2번을 못 뒤집음).\n\n"
    "【절대 규칙】\n"
...
_DRAFT_BAN_RULES = (
    ...
    "★ 양팀 확정 픽이 0~2개(1페이즈 밴): 조합이 없으니 '챔피언 자체'를 잘라라.\n"
    "  ① 현 패치 OP([현 패치 티어리스트] 최상위·고밴률 챔프) ② 상대 팀 장인 챔프(클랜 전적 판수·승률 높은 픽) 저격.\n"
    "  → 겹치는 챔프(상대 장인 + 현 메타 OP)가 최우선 1순위.\n"
    ...
    "- 상대 데이터가 없으면 없는 대로 [현 패치 티어리스트] 기준으로만 추천하라 — 근거 없는 저격 밴을 지어내지 마라.\n"
...
def _draft_advise(ctx, my_pool):
    ...
    if is_ban:
        ...
        opp_txt = "\n".join(
            ...
            for nm, champs in _opp if champs) or ("(상대 전적 정보 없음 — ★이 경우 상대 저격 밴은 불가능하다.\n"
                                                  " [현 패치 티어리스트]의 최상위·고밴률 챔프 기준으로만 추천하고, 그 사실을 이유에 밝혀라)")
        if _bphase == 1:
    ...
    system_text = ((_DRAFT_BAN_RULES if is_ban else _DRAFT_RULES)
                   + (("\n\n[현 패치 티어리스트 — op.gg " + _PATCH_META["patch"] + " 에메랄드+ 솔로랭크, 포지션별 랭킹순(앞일수록 강함)]\n" + _pm_txt)
                      if (_pm_txt := re.sub(r"T\d+·", "", _patch_meta_text() or "")) else "")   # 구캐시의 T코드도 주입 전 제거
                   + "\n\n[클랜 내전 챔피언 메타(판수순)]\n" + _clan_meta_lines())
